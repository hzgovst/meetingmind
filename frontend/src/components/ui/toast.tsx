...
export function toast(props: Omit<ToastState, 'id'>) {
  ...
  dispatch([...toastMemory, newToast]);
  setTimeout(() => {
    dispatch(toastMemory.filter((t) => t.id !== id));
  }, 5000);
}

export function useToast() {
  const [toasts, setToasts] = React.useState<ToastState[]>(toastMemory);
